...
def build_hq_monthly_pdf(
    *,
    ...
    elements.append(Paragraph("<b>Statut :</b> PERIODE GELEE", styles["Normal"]))
    ...
    for (
        ...
        table_data.append(
            [
                ...
                city_name or "",
                deliveries or 0,
                f"{Decimal(str(total_subvention_due)):.2f}",
                f"{Decimal(str(total_volume_chf)):.2f}",
            ]
        )
        total_deliveries += int(deliveries or 0)
        total_subvention += Decimal(str(total_subvention_due or 0))
        total_volume += Decimal(str(total_volume_chf or 0))
    ...
    elements.append(
        Paragraph(
            f"Total subvention : CHF {total_subvention:.2f}",
            styles["Normal"],
        )
    )
    elements.append(
        Paragraph(f"Total facture : CHF {total_volume:.2f}", styles["Normal"])
    )
